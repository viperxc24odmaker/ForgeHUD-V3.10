...
public class HudTrackers {
	...
	public static void countFrame() {
		...
		pushHistory(fps, now);
		frameTimeMs = fps > 0 ? 1000.0D / fps : 0.0D;
	}
	...
	// ---- ping history + frame time ----
	private static final int[] pingHistory = new int[HISTORY];
	private static int pingIndex = 0;
	private static long lastPingPush = 0L;
	private static double frameTimeMs = 0.0D;

	public static void pushPing(Minecraft client, long now) {
		if (now - lastPingPush < 500L) return;
		lastPingPush = now;
		pingHistory[pingIndex] = ping(client);
		pingIndex = (pingIndex + 1) % HISTORY;
	}

	public static int[] pingHistory() {
		int[] ordered = new int[HISTORY];
		for (int i = 0; i < HISTORY; i++) {
			ordered[i] = pingHistory[(pingIndex + i) % HISTORY];
		}
		return ordered;
	}

	public static double frameTimeMs() {
		return frameTimeMs;
	}
