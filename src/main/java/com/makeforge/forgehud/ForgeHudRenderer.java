...
public class ForgeHudRenderer {
	...
	public static void render(GuiGraphics graphics) {
		...
		// HUD scaling. Everything below draws in scaled space; dividing the screen size by the
		// same factor keeps every anchor mapping to the same physical spot on screen.
		float scale = Mth.clamp(config.hudScale, 0.5F, 2.0F);
		...
		int realWidth = client.getWindow().getGuiScaledWidth();
		int realHeight = client.getWindow().getGuiScaledHeight();
		int screenWidth = Math.round(realWidth / scale);
		int screenHeight = Math.round(realHeight / scale);
		...
		drawModule(graphics, client, HudModule.CHUNK, screenWidth, screenHeight, chunkLines(client));
		drawModule(graphics, client, HudModule.WEATHER, screenWidth, screenHeight, weatherLines(client));
		drawModule(graphics, client, HudModule.VITALS, screenWidth, screenHeight, vitalsLines(client));
		drawModule(graphics, client, HudModule.XP, screenWidth, screenHeight, xpLines(client));
		drawModule(graphics, client, HudModule.HELD, screenWidth, screenHeight, heldLines(client));
		drawModule(graphics, client, HudModule.STATE, screenWidth, screenHeight, stateLines(client));
		drawModule(graphics, client, HudModule.FRAMETIME, screenWidth, screenHeight, frameTimeLines());
		drawModule(graphics, client, HudModule.MEMORY, screenWidth, screenHeight, memoryLines());
		drawModule(graphics, client, HudModule.PLAYERS, screenWidth, screenHeight, playerLines(client));
		drawModule(graphics, client, HudModule.SERVER, screenWidth, screenHeight, serverLines(client));
		...
		if (config.module(HudModule.FPS_GRAPH).enabled) {
			drawGraph(graphics, client, HudModule.FPS_GRAPH, screenWidth, screenHeight,
					HudTrackers.fpsHistory(), HudTrackers.fps() + " fps");
		}
		if (config.module(HudModule.NETGRAPH).enabled) {
			drawGraph(graphics, client, HudModule.NETGRAPH, screenWidth, screenHeight,
					HudTrackers.pingHistory(), HudTrackers.ping(client) + " ms");
		}
		if (config.module(HudModule.DAYCYCLE).enabled) {
			drawDayCycle(graphics, client, screenWidth, screenHeight);
		}
		if (config.module(HudModule.CROSSHAIR).enabled) {
		...
		drawModule(graphics, client, HudModule.REACH, screenWidth, screenHeight, reachLines(client));
		drawModule(graphics, client, HudModule.THREAT, screenWidth, screenHeight, threatLines(client));

		if (config.module(HudModule.ATTACK).enabled) {
			drawChargeBar(graphics, client, HudModule.ATTACK, screenWidth, screenHeight,
					attackCharge(client), "Attack");
		}
		if (config.module(HudModule.BOW).enabled) {
			drawChargeBar(graphics, client, HudModule.BOW, screenWidth, screenHeight,
					bowCharge(client), "Bow");
		}
		...
		// Notifications stay at native size so they are always readable.
		ForgeToasts.render(graphics, client, realWidth);
	}
	...
	/** Shared graph renderer used by both the FPS graph and the net graph. */
	private static void drawGraph(GuiGraphics graphics, Minecraft client, HudModule module,
	                              int screenWidth, int screenHeight, int[] history, String label) {
		int width = 80;
		int height = 34;
		...
		int panelColor = ForgeTheme.hudPanelColor();
		if (panelColor != 0) {
			graphics.fill(x - 3, y - 2, x + width + 3, y + height + 2, panelColor);
			ForgeTheme.accentBar(graphics, x - 3, y - 2, height + 4,
					ForgeTheme.withAlpha(ForgeTheme.accent(), 0xCC));
		}
		...
		graphics.fill(x, graphTop + graphHeight, x + width, graphTop + graphHeight + 1,
				ForgeTheme.withAlpha(accent, 0x66));

		int bars = Math.min(history.length, width / 2);
		int barWidth = Math.max(1, width / Math.max(1, bars));

		for (int i = 0; i < bars; i++) {
			int sample = history[history.length - bars + i];
			int barHeight = Math.round(graphHeight * (sample / (float) peak));
			int barX = x + i * barWidth;
			int shade = ForgeTheme.lerpColor(ForgeTheme.withAlpha(accent, 0x55),
					ForgeTheme.withAlpha(accent, 0xEE), sample / (float) peak);
			graphics.fill(barX, graphTop + graphHeight - barHeight, barX + barWidth - 1,
					graphTop + graphHeight, shade);
		}

		graphics.drawString(client.font, label + "  peak " + peak, x, y,
	...
	private static List<String> chunkLines(Minecraft client) {
		BlockPos pos = client.player.blockPosition();
		List<String> lines = new ArrayList<>();
		lines.add("Chunk " + (pos.getX() >> 4) + " " + (pos.getZ() >> 4));
		lines.add("In chunk " + (pos.getX() & 15) + " " + (pos.getZ() & 15));
		return lines;
	}
	...
	private static List<String> weatherLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			String weather = client.level.isThundering() ? "Thunder"
					: (client.level.isRaining() ? "Rain" : "Clear");
			lines.add("Weather " + weather);
			// Vanilla derives the moon phase as (dayTime / 24000) % 8. Computing it here
			// instead of calling getMoonPhase() keeps this off an API that moved in 1.21.11.
			int phase = (int) Math.floorMod(client.level.getDayTime() / 24000L, 8L);
			lines.add("Moon " + MOON_PHASES[Math.floorMod(phase, MOON_PHASES.length)]);
		} catch (Throwable ignored) {
		}
		return lines;
	}

	private static List<String> vitalsLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			LocalPlayer player = client.player;
			lines.add("Health " + Math.round(player.getHealth()) + "/" + Math.round(player.getMaxHealth()));
			lines.add("Hunger " + player.getFoodData().getFoodLevel() + "/20");
			float absorption = player.getAbsorptionAmount();
			if (absorption > 0.0F) {
				lines.add("Absorb " + Math.round(absorption));
			}
		} catch (Throwable ignored) {
		}
		return lines;
	...
	private static List<String> xpLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			LocalPlayer player = client.player;
			lines.add("Level " + player.experienceLevel);
			lines.add("Progress " + Math.round(player.experienceProgress * 100.0F) + "%");
		} catch (Throwable ignored) {
		}
		return lines;
	}

	private static List<String> heldLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			...
			if (stack.getCount() > 1) {
				...
				int max = Math.max(1, stack.getMaxDamage());
				lines.add("Durability " + (max - stack.getDamageValue()) + "/" + max);
			}
		} catch (Throwable ignored) {
		}
		return lines;
	...
	private static List<String> stateLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			LocalPlayer player = client.player;
			StringBuilder builder = new StringBuilder();
			if (player.isSprinting()) builder.append("SPRINT ");
			if (player.isCrouching()) builder.append("SNEAK ");
			if (player.isSwimming()) builder.append("SWIM ");
			if (player.isFallFlying()) builder.append("FLY ");
			if (player.isOnFire()) builder.append("FIRE ");
			if (player.isInWater()) builder.append("WATER ");
			String state = builder.toString().trim();
			if (!state.isEmpty()) lines.add(state);
		} catch (Throwable ignored) {
		}
		return lines;
	}

	private static List<String> frameTimeLines() {
		double ms = HudTrackers.frameTimeMs();
		return List.of("Frame " + (Math.round(ms * 10.0D) / 10.0D) + " ms");
	}

	private static List<String> memoryLines() {
		Runtime runtime = Runtime.getRuntime();
		long max = runtime.maxMemory() / 1048576L;
		long used = (runtime.totalMemory() - runtime.freeMemory()) / 1048576L;
		long percent = max > 0 ? (used * 100L / max) : 0L;
		List<String> lines = new ArrayList<>();
		lines.add("Memory " + used + "/" + max + " MB");
		lines.add("Used " + percent + "%");
		return lines;
	}

	private static List<String> playerLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			var players = client.level.players();
			lines.add("Nearby " + Math.max(0, players.size() - 1));
			int shown = 0;
			for (var other : players) {
				if (other == client.player) continue;
				if (shown >= 4) break;
				double distance = Math.sqrt(other.distanceToSqr(client.player));
				lines.add(other.getName().getString() + " " + Math.round(distance) + "m");
				shown++;
			}
		} catch (Throwable ignored) {
		}
	...
	private static List<String> serverLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		String world = HudTrackers.worldId(client);
		lines.add(world.equals("singleplayer") ? "Singleplayer" : world);
		try {
			lines.add("Online " + client.level.players().size());
		} catch (Throwable ignored) {
		}
		return lines;
	}

	private static void drawDayCycle(GuiGraphics graphics, Minecraft client, int screenWidth, int screenHeight) {
		int width = 110;
		int height = 16;
		int x = anchorX(HudModule.DAYCYCLE, screenWidth, width);
		int y = anchorY(HudModule.DAYCYCLE, screenHeight, height);

		long time = Math.floorMod(client.level.getDayTime(), 24000L);
		float progress = time / 24000.0F;

		graphics.fill(x - 1, y - 1, x + width + 1, y + 11, 0x99070B12);
		ForgeTheme.gradientH(graphics, x, y, width / 2, 10, 0xFF2B6FB5, 0xFF7EC8FF);
		ForgeTheme.gradientH(graphics, x + width / 2, y, width - width / 2, 10, 0xFF1B2340, 0xFF0A0E1A);

		int marker = x + Math.round(progress * width);
		graphics.fill(marker - 1, y - 2, marker + 2, y + 12, 0xFFFFFFFF);

		boolean day = time < 12000L;
		String label = (day ? "Day " : "Night ") + (client.level.getDayTime() / 24000L);
		graphics.drawString(client.font, label, x, y + 12, HudColors.current(), HudConfig.get().textShadow);
	}

	private static void drawCrosshair(GuiGraphics graphics, int screenWidth, int screenHeight) {
		int cx = screenWidth / 2;
		int cy = screenHeight / 2;
		int color = ForgeTheme.accent();

		switch (HudConfig.get().crosshairStyle) {
			case 1 -> graphics.fill(cx - 1, cy - 1, cx + 1, cy + 1, color);
			case 2 -> {
				for (int i = -3; i <= 3; i++) {
					graphics.fill(cx + i, cy - 4, cx + i + 1, cy - 3, color);
					graphics.fill(cx + i, cy + 3, cx + i + 1, cy + 4, color);
					graphics.fill(cx - 4, cy + i, cx - 3, cy + i + 1, color);
					graphics.fill(cx + 3, cy + i, cx + 4, cy + i + 1, color);
				}
			}
			case 3 -> {
				graphics.fill(cx - 6, cy - 6, cx - 2, cy - 5, color);
				graphics.fill(cx - 6, cy - 6, cx - 5, cy - 2, color);
				graphics.fill(cx + 2, cy - 6, cx + 6, cy - 5, color);
				graphics.fill(cx + 5, cy - 6, cx + 6, cy - 2, color);
				graphics.fill(cx - 6, cy + 5, cx - 2, cy + 6, color);
				graphics.fill(cx - 6, cy + 2, cx - 5, cy + 6, color);
				graphics.fill(cx + 2, cy + 5, cx + 6, cy + 6, color);
				graphics.fill(cx + 5, cy + 2, cx + 6, cy + 6, color);
			}
			default -> {
				graphics.fill(cx - 5, cy, cx - 1, cy + 1, color);
				graphics.fill(cx + 1, cy, cx + 5, cy + 1, color);
				graphics.fill(cx, cy - 5, cx + 1, cy - 1, color);
				graphics.fill(cx, cy + 1, cx + 1, cy + 5, color);
			}
		}
	}
	...
	private static List<String> reachLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			if (client.crosshairPickEntity != null) {
				double distance = client.player.distanceTo(client.crosshairPickEntity);
				lines.add("Reach " + (Math.round(distance * 100.0D) / 100.0D) + "m");
			}
		} catch (Throwable ignored) {
		}
		return lines;
	...
	private static List<String> threatLines(Minecraft client) {
		List<String> lines = new ArrayList<>();
		try {
			LivingEntity closest = null;
			double best = Double.MAX_VALUE;
			for (var other : client.level.players()) {
				if (other == client.player) continue;
			...
			if (closest == null) return lines;

			lines.add("Threat " + closest.getName().getString());
			lines.add(Math.round(closest.getHealth()) + " hp  " + Math.round(best) + "m");
		} catch (Throwable ignored) {
		}
		return lines;
	}

	private static float attackCharge(Minecraft client) {
		try {
			return Mth.clamp(client.player.getAttackStrengthScale(0.0F), 0.0F, 1.0F);
		} catch (Throwable t) {
			return 1.0F;
		}
	}

	private static float bowCharge(Minecraft client) {
		try {
			if (!client.player.isUsingItem()) return 0.0F;
			return Mth.clamp(client.player.getTicksUsingItem() / 20.0F, 0.0F, 1.0F);
		} catch (Throwable t) {
			return 0.0F;
		}
	}

	private static void drawChargeBar(GuiGraphics graphics, Minecraft client, HudModule module,
	                                  int screenWidth, int screenHeight, float charge, String label) {
		int width = 80;
		int height = 16;
		int x = anchorX(module, screenWidth, width);
		int y = anchorY(module, screenHeight, height);
		...
		int panelColor = ForgeTheme.hudPanelColor();
		if (panelColor != 0) {
			graphics.fill(x - 3, y - 2, x + width + 3, y + height, panelColor);
		}

		boolean full = charge >= 0.999F;
		int fill = Math.round(width * charge);
		int color = full ? 0xFF55FF55 : ForgeTheme.lerpColor(0xFFFF5555, 0xFFFFAA00, charge);

		graphics.drawString(client.font, label + " " + Math.round(charge * 100.0F) + "%", x, y,
				HudColors.current(), HudConfig.get().textShadow);
		graphics.fill(x, y + 10, x + width, y + 14, 0xFF2A323E);
		graphics.fill(x, y + 10, x + fill, y + 14, color);
	}
