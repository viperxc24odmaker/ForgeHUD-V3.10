...
public class HudLayout {
	...
	public static int[] size(HudModule module) {
		return switch (module) {
			...
			case CHUNK -> new int[]{90, 20};
			case WEATHER -> new int[]{80, 20};
			case DAYCYCLE -> new int[]{110, 16};
			case VITALS -> new int[]{90, 20};
			case XP -> new int[]{90, 20};
			case HELD -> new int[]{110, 20};
			case STATE -> new int[]{90, 10};
			case CROSSHAIR -> new int[]{16, 16};
			case NETGRAPH -> new int[]{80, 34};
			case FRAMETIME -> new int[]{70, 10};
			case MEMORY -> new int[]{90, 20};
			case PLAYERS -> new int[]{90, 40};
			case SERVER -> new int[]{110, 20};
			case REACH -> new int[]{80, 10};
			case ATTACK -> new int[]{80, 16};
			case BOW -> new int[]{80, 16};
			case THREAT -> new int[]{100, 20};
		};
	}
	...
	private static final HudModule[] LEFT_STACK = {
			HudModule.INFO, HudModule.NETHER, HudModule.CHUNK, HudModule.NETWORK, HudModule.SPEED,
			HudModule.LIGHT, HudModule.WEATHER, HudModule.STATS, HudModule.DURABILITY
	};

	private static final HudModule[] RIGHT_STACK = {
			HudModule.EFFECTS, HudModule.SERVER, HudModule.PLAYERS, HudModule.WAYPOINTS,
			HudModule.DEATH, HudModule.MEMORY, HudModule.FRAMETIME, HudModule.SESSION,
			HudModule.FPS_GRAPH, HudModule.NETGRAPH
	};
	...
	public static void autoArrange(int screenWidth, int screenHeight) {
		...
		config.module(HudModule.DAYCYCLE).x = 0.5F;
		config.module(HudModule.DAYCYCLE).y = 0.10F;

		// Centre column above the hotbar.
		float centre = 0.62F;
		for (HudModule module : new HudModule[]{HudModule.STATE, HudModule.HELD, HudModule.VITALS, HudModule.XP}) {
			HudConfig.ModuleData data = config.module(module);
			if (!data.enabled) continue;
			data.x = 0.5F;
			data.y = Mth.clamp(centre, 0.0F, 0.92F);
			centre += (size(module)[1] / (float) Math.max(1, screenHeight)) + gap;
		}
